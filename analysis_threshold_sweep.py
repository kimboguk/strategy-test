#!/usr/bin/env python3
"""
근접도 임계값 스윕 (0.90 ~ 1.00, 0.01 단위)
- ATH근접 / 52W근접 각각
- 근접도 필터 + BS 기대수익률 순위 Top 3
- 종목당 100만원
"""
...
def precompute(sd, ed, prox_type):
    """OHLCV 로드 + 근접도 사전계산 + BS 기대수익률 로드 (1회만)"""
    with engine.connect() as conn:
...
def run_with_threshold(ohlcv_lookup, signal_by_ticker, bs_by_date, calendar, threshold):
    """근접도 필터 + BS 기대수익률 순위 Top N"""
    positions = {}
    all_trades = []
    pending_entry = []
...
def calc_metrics(trades):
    ...
    return {'n': n, 'win_rate': win_rate, 'wavg_pnl': wavg,
            'avg_hold': avg_hold, 'sl_pct': sl_pct, 'pf': pf}


thresholds = [round(0.90 + i * 0.01, 2) for i in range(11)]  # 0.90 ~ 1.00
years = [
...
sep = '=' * 86
print(sep)
print('  근접도 임계값 스윕 (전일 종가 판단 -> 다음날 시가, 종목당 100만원, 일별 Top 1)')
print('  근접도 필터 + BS 기대수익률 순위')
print('  SL: -20% / TP: +10% 시작 +5% 간격 50% 부분청산')
print(sep)

for prox_type, prox_label in [('ath', 'ATH'), ('52w', '52W')]:
    print(f'\n  ── {prox_label} 근접 + BS순위 ──')
    ...
    for year, sd, ed in years:
        ...
        for th in thresholds:
            trades = run_with_threshold(ohlcv, signals, bs_data, cal, th)
